...
def cal_cv_error(algorithm, cv_num, signal, dictionary):
    ...
    for i in range(cv_num):
        ...
        error_lst.append(algorithm.score(test_dictionary, test_signal))
...
def cv_best_K(signal, dictionary, cv_num, K_lst):
    ...
    for K in K_lst:
        MP_tmp = MP(K)
        K_cv_error.append(cal_cv_error(MP_tmp, cv_num, signal, dictionary))
...
def cv_best_K_noise_level_multi_trial(N, d, m, noise_level_lst, cv_num, K_lst, trial_num, output_filename = None):
    ...
    for noise_level in noise_level_lst:
        ...
        for trial in range(trial_num):
            ...
            best_estimator_tmp = MP(int(lowest_error_K))
            best_estimator_tmp.fit(y_train, X_train)
            training_error = best_estimator_tmp.score(y_train, X_train)
            testing_score = best_estimator_tmp.score(y_test, X_test)
            testing_score_tmp.append(testing_score)
            training_score_tmp.append(training_error)
            trials_best_K_tmp.append(lowest_error_K)
            MSE_loweset_K_temp.append(lowest_error)
            print("Trial: ", trial, " Best K: ", lowest_error_K, " Lowest Error: ", lowest_error, " Training Error: ", training_error,  " Testing Score: ", testing_score)
        ...
        noise_level_training_score.append(np.mean(training_score_tmp))
        print("Average best K for noise level: ", noise_level, " is: ", np.mean(trials_best_K_tmp), " with CV Error: ", np.mean(MSE_loweset_K_temp), " , training score: ", np.mean(training_score_tmp)," and testing score: ", np.mean(testing_score_tmp))
    res_log['noise_level_best_K'] = noise_level_best_K
    res_log['noise_level_lowest_MSE'] = noise_level_lowest_MSE
    res_log['noise_level_testing_score'] = noise_level_testing_score
    res_log['noise_level_training_score'] = noise_level_training_score
    with open('./memory/' + output_filename, 'wb') as f:
        pkl.dump(res_log, f)
    print("Finished!")
    print("Log file saved to: ", './memory/' + output_filename)
    return noise_level_best_K, noise_level_lowest_MSE, noise_level_testing_score, res_log
...
if __name__ == "__main__":
    (
        ...
        K_lst,
        trial_num,
        output_filename="omp_baseline_testing_300_600_20.pkl",
    )
